...
def ReadResponseTable():
    ...
    vAR_sql =(
        "select * from `"+ os.environ["GCP_PROJECT_ID"]+"."+os.environ["GCP_BQ_SCHEMA_NAME"]+"."+vAR_response_table_name+"`"+ " where date(created_dt)=current_date() and RUN_ID=(select max(RUN_ID) from `" +os.environ["GCP_PROJECT_ID"]+"."+os.environ["GCP_BQ_SCHEMA_NAME"]+"."+vAR_response_table_name+"` where date(created_dt)=current_date()"+") order by PLATE_TYPE_COUNT DESC,LICENSE_PLATE_DESC ASC,LICENSE_PLATE_CONFIG ASC"
    )
